...
//	public bool NearMode = false;
	...
	public TextMeshProUGUI GesturesDebugText;
	...
	public UnityEvent<uint> OnUserAdded;
	public UnityEvent<uint> OnUserRemoved;
	...
	void Update()
	{
		if(KinectInitialized)
		{
			...
			if (Player1Calibrated)
			{
				...
				{
					if (player1Gestures[i].complete)
					{
						if (player1Gestures[i].gesture == KinectGestures.Gestures.Click)
						{
						...
						{
							if (listener.GestureCompleted(Player1ID, 0, player1Gestures[i].gesture,
								(KinectWrapper.NuiSkeletonPositionIndex) player1Gestures[i].joint,
								player1Gestures[i].screenPos))
							{
								ResetPlayerGestures(Player1ID);
							}
						}
					}
					else if (player1Gestures[i].cancelled)
					{
						foreach (KinectGestures.GestureListenerInterface listener in gestureListeners)
						{
							if (listener.GestureCancelled(Player1ID, 0, player1Gestures[i].gesture,
								(KinectWrapper.NuiSkeletonPositionIndex) player1Gestures[i].joint))
							{
								ResetGesture(Player1ID, player1Gestures[i].gesture);
							}
						}
					}
					else if (player1Gestures[i].progress >= 0.1f)
					{
						if ((player1Gestures[i].gesture == KinectGestures.Gestures.RightHandCursor ||
						     player1Gestures[i].gesture == KinectGestures.Gestures.LeftHandCursor ||
						     player1Gestures[i].gesture == KinectGestures.Gestures.RightHandCursorBothSides) &&
						    player1Gestures[i].progress >= 0.5f)
						{
							if (player1Gestures[i].gesture == KinectGestures.Gestures.RightHandCursorBothSides)
							{
								if (HandCursor1 != null)
								{
									Vector3 vCursorPos = player1Gestures[i].screenPos;

									if (HandCursor1 == null)
									{
										float zDist = HandCursor1.transform.position.z -
									...
									HandCursor1.transform.position = Vector3.Lerp(HandCursor1.transform.position,
										vCursorPos, 3 * Time.deltaTime);
								}

								if (ControlMouseCursor)
								{
									Vector3 vCursorPos = HandCursor1 != null
										? HandCursor1.transform.position
							...
							if (GetGestureProgress(player1Gestures[i].userId, KinectGestures.Gestures.Click) < 0.3f)
							{
								if (HandCursor1 != null)
								{
									Vector3 vCursorPos = player1Gestures[i].screenPos;
						...
						foreach (KinectGestures.GestureListenerInterface listener in gestureListeners)
						{
							listener.GestureInProgress(Player1ID, 0, player1Gestures[i].gesture,
								player1Gestures[i].progress,
								(KinectWrapper.NuiSkeletonPositionIndex) player1Gestures[i].joint,
								player1Gestures[i].screenPos);
						}
					}
				...
				foreach(KinectGestures.GestureData gestureData in player2Gestures)
				{
					...
					else if(gestureData.progress >= 0.1f)
					{
						if((gestureData.gesture == KinectGestures.Gestures.RightHandCursor || 
							...
							gestureData.gesture == KinectGestures.Gestures.RightHandCursorBothSides) && 
